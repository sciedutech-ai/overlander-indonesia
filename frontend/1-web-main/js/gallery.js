document.addEventListener("DOMContentLoaded", () => {
    ...
    // ==========================================
    // GALLERY MASONRY FILTER LOGIC (Dibuat Global)
    // ==========================================
    window.initGalleryFilter = function() {
        ...
        if (filterItems.length > 0 && masonryItems.length > 0) {
            ...
            // Bersihkan event listener lama (jika dipanggil ulang)
            filterItems.forEach(filter => {
                const newFilter = filter.cloneNode(true);
                filter.parentNode.replaceChild(newFilter, filter);
            });

            // Re-select filter items yang sudah di-clone
            const updatedFilterItems = document.querySelectorAll('.filter-item');

            updatedFilterItems.forEach(filter => {
                filter.addEventListener('click', function() {
                    
                    // 1. Hapus class 'active' dari semua tombol, lalu tambahkan ke tombol yang diklik
                    updatedFilterItems.forEach(item => item.classList.remove('active'));
